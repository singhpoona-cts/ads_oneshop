...
class MerchantAccountsTest(absltest.TestCase):
  ...
  def test_discover_topology_mca_with_inaccessible_parent_profile(self):
    """An MCA with inaccessible root profile but accessible sub-accounts."""
    clients = mock.MagicMock()
    clients.accounts.account_path.side_effect = lambda x: f'accounts/{x}'

    # get_account fails with PermissionDenied
    clients.accounts.get_account.side_effect = (
        gax_exceptions.PermissionDenied('Forbidden')
    )
    ...
    self.assertEqual(
        base_accounts_by_id,
        {'100': None, '101': child_1, '102': child_2},
    )

  def test_fetch_account_row_with_none_base_account(self):
    """If base_account is None, _fetch_account_row should not raise Error."""
    clients = mock.MagicMock()
    clients.accounts.account_path.side_effect = lambda x: f'accounts/{x}'
    # Mock return values for list_account_services, list_users, etc.
    clients.services.list_account_services.return_value = []
    clients.users.list_users.return_value = []
    clients.relationships.list_account_relationships.return_value = []
    clients.homepage.get_homepage.return_value = None
    clients.business_info.get_business_info.return_value = None
    clients.business_identity.get_business_identity.return_value = None
    (clients.automatic_improvements
     .get_automatic_improvements.return_value) = None

    row = merchant_accounts._fetch_account_row(
        clients=clients,
        account_id='100',
        base_account=None,
        parent=None,
    ...
    self.assertEqual(row.account_id, 100)
    self.assertTrue(row.is_advanced)
    self.assertFalse(row.HasField('account_name'))
