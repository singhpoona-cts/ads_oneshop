...
class _FakeProductsClient:
  """Minimal stand-in for ProductsServiceClient that counts pager consumption."""
...
class ListLeafProductsStreamingTest(absltest.TestCase):
  """A large merchant's catalog must never be materialized in memory at once."""
  ...
  def test_download_products_streams_to_disk(self):
    ...
    with mock.patch.object(
        mp, 'ProductsServiceClient', return_value=client
    ):
