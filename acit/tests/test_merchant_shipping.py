...
"""Tests for the Merchant API v1 shipping-settings ingestion helpers."""
...
class MerchantShippingTest(absltest.TestCase):
  ...
  def test_to_dict_renders_enums_as_strings(self):
    ...
    It must use enum *names*, not integers, for BigQuery STRING.
    """
    settings = ma.ShippingSettings(services=[
        _service(
            service_name='FedEx',
    ...
    self.assertEqual(
        svc['delivery_time']['handling_business_day_config']
        ['business_days'], ['MONDAY', 'FRIDAY'])

  def test_to_dict_uses_snake_case_keys(self):
    """Tests that _to_dict.

    Check if it converts protobuf fields to snake_case dictionary keys.
    """
    settings = ma.ShippingSettings(services=[
        _service(
            service_name='Main',
  ...
  def test_main_table_cell_flat_rate(self):
    ...
    settings = ma.ShippingSettings(services=[
        _service(rate_groups=[
            ma.RateGroup(main_table=ma.Table(rows=[
                ma.Row(cells=[
                    ma.Value(flat_rate={
