...
class TestAcit(parameterized.TestCase):
    ...
    def setUp(self):
        ...
        self.enter_context(
            mock.patch.object(acit.merchant_products,
                              'download_products',
                              autospec=True))
        self.enter_context(
            mock.patch.object(acit.merchant_lia,
                              'download_omnichannel_settings',
                              autospec=True))
        self.enter_context(
            mock.patch.object(acit.merchant_shipping,
                              'download_shipping_settings',
                              autospec=True))
        ...
        with mock.patch.object(client,
            ...
            with flagsaver.as_parsed(
                ...
                self.assertEqual('123',
                                 run_query.call_args.kwargs['customer_id'])
    ...
    def test_credentials(
        ...
        with mock.patch.object(client,
            ...
            output_dir = self.create_tempdir()
            with flagsaver.as_parsed(output=str(output_dir),):
                acit.main(None)
                if expect_is_adc:
