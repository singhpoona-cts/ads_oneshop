...
flags.DEFINE_string('source_dir', '/tmp/acit/*',
                    'The root path for all source files.')

flags.DEFINE_string('products_output', 'out.jsonlines',
...
def _ReadGoogleAdsRows(description: str, path: str) -> beam.ParDo:
    ...
    return textio.ReadFromText(
        path) | f'{description} to Google Ads Row' >> beam.Map(json.loads)
...
def combine_campaign_settings(
    ...
    """Creates a single record for each campaign
    with its targeted languages and listing scopes.
  ...
  Args:
    ...
    languages_by_campaign_id: The PTable of campaign ID and language targeting
      information.
    listing_scopes_by_campaign_id: The PTable of campaign ID and (at most one)
      root listing scope.
...
def main(argv):
    ...
    with pipeline.Pipeline(options=opts) as p:
        ...
        asset_group_listing_filters = (
            p | 'Read Asset Group Listing Filters' >> _ReadGoogleAdsRows(
        ...
        category_names_by_id = (
            p | 'Read Product Categories' >> _ReadGoogleAdsRows(
                'Product Categories',
                ...
                next(
                    iter([
                        localization['value'] for localization in row[
                            'productCategoryConstant']['localizations'] if
                        localization['regionCode'] == 'US' and localization[
                            'languageCode'] == 'en'
                    ])),
            )))

        language_codes_by_resource_name = (
            p | 'Read Language Codes' >> _ReadGoogleAdsRows(
                'Language Codes',
        ...
        products = (p | 'Read Products' >> textio.ReadFromText(
        ...
        _ = (p | 'Glob LIA Settings files' >> fileio.MatchFiles(
            file_pattern=(
      ...
      """
        ...
        all_products = (
            ...
            # Add Shopping targeting. Side-input
            # views provide in-memory lookups.
        ...
        _ = (all_products | 'Convert to final products table output' >>
             beam.Map(products_table_row) | 'JSON' >> beam.Map(json.dumps) |
